...
func NewTelemetry(meter metric.Meter) *Telemetry {
	...
	if cec, err := telemetry.NewDeferrableInt64Counter(meter, "ottl.conditions.error.count", nil, nil); err == nil {
		ret.ConditionsErrorCounter = cec
	}
	if cec, err := telemetry.NewDeferrableInt64Counter(meter, "ottl.conditions.evaluated.count", nil, nil); err == nil {
		ret.ConditionsEvaluatedCounter = cec
	}
	if ceh, err := telemetry.NewDeferrableHistogram(meter, "ottl.conditions.evaluated.time", nil, nil); err == nil {
		ret.ConditionsEvaluatedHistogram = ceh
	}
	if rlc, err := telemetry.NewDeferrableInt64Counter(meter, "ottl.rate_limited.count", nil, nil); err == nil {
		ret.RateLimitedCounter = rlc
	}
	if sec, err := telemetry.NewDeferrableInt64Counter(meter, "ottl.statements.error.count", nil, nil); err == nil {
		ret.StatementsErrorCounter = sec
	}
	if seh, err := telemetry.NewDeferrableHistogram(meter, "ottl.statements.executed.time", nil, nil); err == nil {
		ret.StatementsExecutedHistogram = seh
	}
	if sec, err := telemetry.NewDeferrableInt64Counter(meter, "ottl.statements.executed.count", nil, nil); err == nil {
